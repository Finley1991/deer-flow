...
description: "Interact with DeepHourAI AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to DeepHourAI for research/analysis, start a DeepHourAI conversation thread, check DeepHourAI status or health, list available models/skills/agents in DeepHourAI, manage DeepHourAI memory, upload files to DeepHourAI threads, or delegate complex research tasks to DeepHourAI. Also use when the user mentions deerflow, deer flow, or wants to run a deep research task that DeepHourAI can handle."
---

# DeepHourAI Skill

Communicate with a running DeepHourAI instance via its HTTP API. DeepHourAI is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

DeepHourAI exposes two API surfaces behind an Nginx reverse proxy:
...
- If health check fails, DeepHourAI is not running. Inform the user they need to start it.
