...
DeepHourAI can use a remote OpenViking server as an optional long-term memory
...
package instead of implementing OpenViking's HTTP protocol inside DeepHourAI.

## Current scope

The first official-adapter integration deliberately preserves DeepHourAI's
existing automatic-memory behavior:

- memory is recalled through DeepHourAI's existing fixed memory query;
...
- one recorder-owned SDK client is shared with retrieval and closed through
  DeepHourAI's existing memory shutdown contract.
...
This version is for one DeepHourAI user backed by one ordinary OpenViking **USER
API key**. OpenViking derives the account and user from that credential.
DeepHourAI does not configure trusted account/user headers and must not receive a
root key for normal memory traffic.

The supported server configuration is OpenViking `api_key` mode, where the USER
key determines the account and user. DeepHourAI supplies its URL and API key
...
`OPENVIKING_USER` values from DeepHourAI's repository-root `.env` and service
...
`owner_user_id` binds the configured key to one DeepHourAI identity. Use
`default` when DeepHourAI authentication is disabled. In an authenticated
single-user deployment, use that user's DeepHourAI ID. A request for another
DeepHourAI user is rejected before OpenViking is contacted, preventing one USER
...
## Configure DeepHourAI

Create or select an OpenViking user, then copy its USER API key into DeepHourAI's
repository-root `.env`:
...
For a host-installed OpenViking used by Docker DeepHourAI, set `base_url` to
...
The dependency on `langchain-openviking==0.1.0` is declared by DeepHourAI's
...
Then start DeepHourAI normally:
...
DeepHourAI is available at <http://localhost:2026>. OpenViking Studio is
...
management flow. Only that USER key belongs in DeepHourAI's
`OPENVIKING_API_KEY` variable.

## Identity and session mapping

One DeepHourAI thread maps deterministically to one OpenViking Session. A commit
...
The default DeepHourAI agent uses `default_peer_id` (`deerflow` by default).
...
- `read: fail_open` logs retrieval failures and continues the turn without
  ...
  The same policy applies at DeepHourAI's 5-second async injection deadline,
...
- DeepHourAI stores only hashes and counters in a bounded local capture cursor
...
- Graceful shutdown stops new memory work, waits up to
  ...
  recorder-owned SDK client. It does not introduce a new DeepHourAI lifecycle or
  background worker.
