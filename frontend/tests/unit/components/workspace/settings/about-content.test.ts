...
test("aboutMarkdown heading interpolates the app version", async () => {
  ...
  expect(aboutMarkdown).toContain("[About DeepHourAI 9.9.9-test]");
  // Milestone copy in the acknowledgments refers to the 1.0/2.0 product
  // generations and must NOT be parameterized.
  expect(aboutMarkdown).toContain("DeepHourAI 1.0 and 2.0");
});
...
test("aboutMarkdown heading reflects the package version when env is unset", async () => {
  ...
  // empty or undefined APP_VERSION interpolation (`About DeepHourAI ]` /
  // `About DeepHourAI undefined]`), not just removal of the old literal.
  expect(aboutMarkdown).toContain(`[About DeepHourAI ${APP_VERSION}]`);
});
