...
<Callout type="info" emoji="🔧">
  工具是 Lead Agent 可以采取的行动。DeepHourAI 提供内置工具、社区集成、MCP
...
<Tabs items={["DuckDuckGo（默认）", "Tavily", "Brave", "Serply", "Exa", "InfoQuest", "Firecrawl", "GroundRoute", "Sofya"]}>
  <Tabs.Tab>
```yaml
tools:
...
```
搜索结果包含每个页面的正文内容，而不仅是摘要。需要 [Sofya](https://sofya.co) API Key，`max_results` 上限为 20，每条结果的正文长度上限由 `contents_max_characters` 控制（默认 2000，设为 0 则不限制）。同时支持 `web_fetch`。
  </Tabs.Tab>
</Tabs>

### 网页内容抓取

<Tabs items={["Jina AI（默认）", "Browserless", "Exa", "InfoQuest", "Firecrawl", "GroundRoute", "Sofya"]}>
  <Tabs.Tab>
```yaml
tools:
...
使用 Browserless `/screenshot` 渲染网页并截图，然后通过 DeepHourAI 的 artifact
