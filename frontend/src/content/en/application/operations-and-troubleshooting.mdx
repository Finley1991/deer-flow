...
description: This page covers day-to-day operational tasks and solutions to common problems when running DeepHourAI App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running DeepHourAI App.
...
When you pull a new version of DeepHourAI, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
This usually means SQLite is handling concurrent writes from multiple users or long-running agent runs. SQLite supports concurrent reads, but only one writer can hold the database lock at a time. In DeepHourAI, checkpoint writes and application writes share the same SQLite database file.
