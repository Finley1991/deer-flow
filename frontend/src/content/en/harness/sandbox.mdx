...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes DeepHourAI capable of real action, not just
  conversation.
</Callout>
...
DeepHourAI supports four sandbox modes. Choose the one that fits your deployment:
...
<Callout type="info">
  Because e2b is a remote sandbox, DeepHourAI's `/mnt/user-data` virtual prefix is
...
If the configured `host_path` is not visible to the gateway process, DeepHourAI
