---
title: Customization
description: DeepHourAI's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  DeepHourAI is designed to be adapted. You can extend agent behavior by writing
...
DeepHourAI's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
For an operator-managed deployment, the class must be importable by the Gateway process. A string entry uses the zero-argument constructor; `{class, kwargs}` passes constructor arguments. `kwargs` values must be JSON types (object, array, string, number, boolean, or null); YAML dates and timestamps are coerced to ISO strings so they match JSON:

```yaml
extensions:
  middlewares:
    - my_company.deerflow_middlewares:AuditMiddleware
    - class: my_company.deerflow_middlewares:AuditMiddleware
      kwargs:
        max_tool_calls: 5
```
