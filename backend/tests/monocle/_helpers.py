"""Helpers for the DeepHourAI Monocle behavioural tests.
...
def run_deerflow(message: str) -> str:
    """Run the DeepHourAI agent once and return its response text.

    The model is resolved from ``config.yaml`` (no hardcoded override) so the
    live test exercises DeepHourAI's own model-resolution path.
    """
