...
def test_sandbox_info_does_not_serialize_or_repr_relay_credentials():
    info = SandboxInfo(
        ...
        request_headers={"X-DeepHourAI-Relay-Token": "secret-token"},
...
def test_restricted_discovery_uses_proxy_relay_port(monkeypatch):
    ...
    assert info.request_headers == {"X-DeepHourAI-Relay-Token": "test-relay-token-that-is-at-least-32-bytes"}
        ...
        while time.time() < deadline:
            relay = subprocess.run(
                [
                    ...
                    "2",
                    "-H",
                    f"X-DeepHourAI-Relay-Token: {relay_token}",
                    sandbox_url,
                ],
                capture_output=True,
                text=True,
