"""Hybrid schema bootstrap for DeepHourAI's application tables.
...
| legacy (DeepHourAI tables, no alembic)          | ``create_all`` (baseline tables only, as backfill) + ``stamp 0001_baseline`` + ``upgrade head`` |
...
DeepHourAI-owned table. ``create_all`` runs first because stamping at
...
def _run_create_all_sync(sync_conn: Any) -> None:
    """Create all DeepHourAI-owned tables on *sync_conn*."""
...
async def _sqlite_lock(engine: AsyncEngine):
    ...
    DeepHourAI. The 30s ``busy_timeout`` plus idempotent revisions cover the
