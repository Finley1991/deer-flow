"""``BoxliteBox`` — DeepHourAI :class:`Sandbox` backed by a BoxLite micro-VM.

DeepHourAI's ``Sandbox`` contract is synchronous; BoxLite's SDK is async-native and
...
``asyncio.to_thread`` worker DeepHourAI invokes us from.
...
class BoxliteBox(Sandbox):
    ...
    Args:
        id: DeepHourAI-side sandbox id (the BoxLite box id).
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        # so DeepHourAI's virtual paths are used as-is; we only reject traversal.
    ...
    def execute_command(
        ...
        DeepHourAI passes a bash command *string*; BoxLite's ``exec`` takes argv, so
