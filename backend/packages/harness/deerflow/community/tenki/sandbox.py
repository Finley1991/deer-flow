"""``TenkiSandbox`` — DeepHourAI :class:`Sandbox` backed by a Tenki cloud sandbox.
...
# ``/mnt`` is root-owned, so DeepHourAI's ``/mnt/user-data`` virtual prefix is not
...
class TenkiSandbox(Sandbox):
    """DeepHourAI Sandbox adapter that delegates to a live Tenki cloud sandbox.

    Args:
        id: DeepHourAI-side sandbox id (the provider's cache key).
    ...
    def _resolve_path(self, path: str) -> str:
        """Map DeepHourAI virtual paths into the writable sandbox home dir.
    ...
    def execute_command(
        ...
        DeepHourAI passes a bash command *string*; it runs through ``sh -lc``.
