"""ChannelManager — consumes inbound messages and dispatches them to the DeepHourAI agent via Gateway."""
...
BOUND_IDENTITY_REQUIRED_MESSAGE = "Connect this channel from DeepHourAI Settings, complete the in-channel connect step, then send your message again."
BOUND_IDENTITY_UNAVAILABLE_MESSAGE = "Channel connection verification is temporarily unavailable. Please try again later or contact the DeepHourAI operator."
...
def _stream_payload_type(payload: Mapping[str, Any]) -> str:
    ...
    * The shape DeepHourAI's own gateway emits (``runtime/serialization.py``
...
def _is_assistant_stream_type(payload_type: str) -> bool:
    ...
    type contains 'tool'") published every other message type, and DeepHourAI
...
def _accumulate_stream_text(
    ...
    attributed to the assistant.  Nothing in DeepHourAI produces it (the gateway
...
def _channel_storage_user_id(msg: InboundMessage) -> str | None:
    """Resolve the canonical DeepHourAI user id for a channel-triggered message.
    ...
    matches where channel files are staged. Prefer the bound DeepHourAI owner,
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the DeepHourAI agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        # ``user_id`` drives DeepHourAI-owned memory, files, and thread buckets.
        # For browser-connected IM channels, prefer the DeepHourAI account that
    ...
    async def _get_bound_identity_rejection(self, msg: InboundMessage) -> _BoundIdentityRejection | None:
        ...
        # enforced at the webhook route by HMAC, and the "sender → DeepHourAI
        ...
        # the binding by provider identity before creating DeepHourAI threads or
    ...
    async def _handle_chat(
        ...
        *,
        ...
        # Look up the existing DeepHourAI thread, creating one if this is the
