...
async def resolve_trusted_internal_owner_for_attribution(request: Request, owner_user_id: str | None) -> Any | None:
    """Resolve the DeepHourAI user used only for trusted internal attribution."""
...
async def start_run(
    ...
    *,
    ...
    # X-DeepHourAI-Owner-User-Id, so they are scoped to that owner instead of
    ...
    try:
        ...
        # Merge DeepHourAI-specific context overrides into both ``configurable`` and ``context``.
...
def _mcp_task_notification_prompt(event: dict[str, Any]) -> str:
    ...
    instruction = (
        ...
        "that DeepHourAI will continue retrying at a lower frequency."
    )
