...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in DeepHourAI 2.0.
...
DeepHourAI uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall DeepHourAI prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with DeepHourAI's main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom DeepHourAI-style prompts**
...
- Custom prompts emphasize the same principles as DeepHourAI's main system prompt (clarity, action-oriented, critical rules)
