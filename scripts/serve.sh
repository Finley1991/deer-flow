#!/usr/bin/env bash
#
# serve.sh — Unified DeepHourAI service launcher
#
# Usage:
...
# so pin DeepHourAI-owned state to the expected backend runtime directory and
...
if ! { \
    ...
    echo "✗ No DeepHourAI config file found."
...
# 1. Gateway API
run_service "Gateway" \
    "cd backend && PYTHONPATH='.:packages/harness:packages/extension-api' uv run --no-sync uvicorn app.gateway.app:app --host 0.0.0.0 --port 8001 $GATEWAY_EXTRA_FLAGS > ../logs/gateway.log 2>&1" \
    8001 30
...
echo ""
echo "=========================================="
echo "  ✓ DeepHourAI is running!  [$MODE_LABEL]"
echo "=========================================="
echo ""
